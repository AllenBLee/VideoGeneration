...
void draw_frame(int t) {
	clear_frame();
	const int pps = 80;
	draw_rect(0 + t * pps, 0 + t * pps, 200, 1000, 0xaa, 0x3a, 0x9b);//origin to south east
	draw_rect(400, 200 - t * pps, 20, 10, 0xff, 0x00, 0xff); //origin to north
	draw_rect(500, 200 + t * pps, 20, 10, 0xff, 0xff, 0x00); //origin to south
	draw_rect(250 + t * pps, 50, 20, 10, 0x00, 0xff, 0xff); //origin to east
...
int main(int argc, char * argv[]) {
	...
	for (int i = 0; i < num_frames; ++i) {
		int time_in_seconds = i / frames_per_second;
